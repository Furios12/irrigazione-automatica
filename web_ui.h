...
.upd-banner{background:#422006;border:1px solid #92400e;color:#fde68a;border-radius:8px;padding:9px 12px;margin-bottom:10px;font-size:.82rem;display:none}
...
#opct{text-align:center;font-size:.75rem;color:#475569;margin-top:4px}
...
<div class="upd-banner" id="upd-banner"></div>
...
<button class="btn bp" onclick="doOTA()">Aggiorna</button>
<div id="op"><progress id="ob" value="0" max="100"></progress><div id="opct">0%</div></div>
<div id="os" class="msg"></div>
</div>

<p class="ver">ESP32 Irrigatore &mdash; %VERSION%</p>
<script>
function msg(id,t,ok){var e=document.getElementById(id);e.textContent=t;e.className='msg '+(ok?'ok':'err');if(ok)setTimeout(function(){e.style.display='none'},3000);}
function saveIrrig(){
...
function doOTA(){
  ...
  x.upload.onprogress=function(e){if(e.lengthComputable){var p=Math.round(e.loaded/e.total*100);document.getElementById('ob').value=p;document.getElementById('opct').textContent=p+'%';}};
  x.onload=function(){msg('os',x.status===200?'Completato! Riavvio...':'Errore: '+x.responseText,x.status===200);};
  x.onerror=function(){msg('os','Errore rete',false);};
  x.send(fd);
}
function checkUpdate(){
  fetch('/update-info').then(function(r){return r.json();}).then(function(d){
    ...
    if(d.available){
      b.innerHTML='&#8593; Aggiornamento disponibile: <b>v'+d.version+'</b>'+(d.notes?' &mdash; '+d.notes:'');
